...
from pages_parsing.detailPagesParse import detailParse
...
def listParse(url_list, dbHandle):
    ...
    if url_list['hasSpider'] == 0:
        ...
        for i in elem:
            ...
            detaillink = i1.find_element_by_class_name('e1').find_element_by_tag_name('a').get_attribute('href')
            print(detaillink)
            position_intro = detailParse(detaillink)
            print(position_intro)
            ...
            sql = "INSERT INTO joblist(company ,position ,adr ,edu ,exp ,lowmoney,highmoney ,avermoney,year,curmon,ta,tag,detaillink,position_intro)" \
                  "values('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')" \
                  % (company, position, adr, edu, exp, int(lowmoney), int(highmoney), avermoney, year, month,
                     url_list['position'], url_list['tag'],detaillink,position_intro)
        ...
        while True:
            ...
            try:
                ...
                sendEmail("状态更新成功" + url_list['link'] + ":{}".format(spiderNum))
                break;
